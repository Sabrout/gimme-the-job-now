...
The idea of tuning normalization parameters for each smartphone is not feasible since the number of smartphone models is increasing everyday. As a result, I suggest we send each
client a color card with an image that supports every normalization operation we need on our platform.

The cost to send a colored card is nothing compared to the gain from automating the normalizaiton process. Once the user has the card, he is required to take a number of pictures
with his smartphone and upload it on the platform.

The platform registers the uploaded image along side the smartphone's model in a seperate table in our database. With having the smartphone's image and the original card, we can
automate the calibration process per smartphone model and publish the process for all users with any supported smartphone model.


**Q3.2 What would you suggest to do to implement data-augmentation ?**
Having worked with data augmentation, I suppose we will use some variant of GAN to increase the images in our training sets. Since, we are generating more training images, it is
better to generate images following the same dimenstions, colors and specifications as normalized images to elemenate the need for normalizing them later.

Although the results generated by GANs can be remarkable, it can be challenging to train a stable model. The reason is that the training process is inherently unstable, resulting
in the simultaneous dynamic training of two competing models. Thus, it is difficult to specify any recommended practices for our platform without further details of the kind of
deep learning models involved.



## Conclusion
To sum up, the report includes descriptions of every file in the repository, the steps to run the server and use the wrapper and the answers to the provided questions. In the case
of getting issues running the scripts or wanting to ask extra questions about my code or my answers, I would be absolutely glad to have a meeting to do so. Many thanks to the team
who prepared such an educational test. It shows how much they care about their work and their standards.
